...
export function OnLoad() {
  ...
  if (saveCorpses) {
    deadMen.forEach((
      c_i,
    ) => (game[c_i.r][c_i.c].circles[c_i.n] = corpse_colors[c_i.color]));
  }
  draw();
}
...
function getSettings() {
  input_players = input_players
    ? input_players
    : document.getElementById("player-count") as HTMLInputElement;
  input_size = input_size
    ? input_size
    : document.getElementById("game-width") as HTMLInputElement;
  check_box = check_box
    ? check_box
    : document.getElementById("checkbox-corpse") as HTMLInputElement;
  input_width = input_width
    ? input_width
    : document.getElementById("game-size") as HTMLInputElement;
  if (!canvas) {
    Log("Added Canvas...");
  ...
  if (game) {
    for (let i = 0; i < game.length; i++) {
      for (let j = 0; j < game.length; j++) game[i][j].recalcPaths();
    }
  }
}
...
function is_cats() {
  ...
  for (let i = 0; i < num_players; i++) {
    player_matches &&= players[i].pieces.every((c) =>
      c.get_match("") == (1 << width) - 1
    );
  }
...
function exchange_colors(
  ...
  if (
    new_c != old_c &&
    (colors.includes(new_c) || corpse_colors.includes(new_c + corpse_alpha))
  ) {
    elem.value = old_c;
    return;
  }
...
function checkWin(circle_info: CircleInfo) {
  ...
  hasMatch ||= check_consecutive(ascending_arr, ascending_arr, playerColor) ||
...
class Player {
  ...
  makeMove(circle_info: CircleInfo) {
    ...
    if (
      played_pos.circles[circle_info.n] == "" && this.markPiece(circle_info.n)
    ) {
      Log(
        `Player ${currentPlayer + 1} moved at ${circle_info.r + 1} ${
          circle_info.c + 1
        } ${circle_info.n + 1}`,
      );
...
class Otrio {
  ...
  recalcPaths() {
    ...
    const p = new Path2D();
    p.arc(
      draw_offset + this.x * spacing,
      draw_offset + this.y * spacing,
      selectR,
      0,
      2 * Math.PI,
    );
...
export default function OtrioGameOld({ username }: Props) {
  user = username;
  useEffect(() => {
    Initilize();
    OnLoad();
  });
  return <canvas id="canvas">Otrio Game</canvas>;
}
